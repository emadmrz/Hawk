        ...
        <div class="row">
            <div class="show-item">
                <div class="col-sm-4 pull-right">
                    <div class="image">
                        <img src="{{ asset('img/icons/store/'.Config::get('addonShop.images')[0]) }}">
                    </div>
                    <div class="other-image">
                        <ul class="row">
                            @foreach(Config::get('addonShop.images') as $image)
                ...
                <div class="col-sm-8 pull-right">
                    {!! Form::open(['route'=>'store.shop.buy', 'method'=>'get']) !!}
                    <div class="title">
                        <h3>{{ Config::get('addonShop.title') }}</h3>
                        <ul class="info">
                            <li><i class="fa fa-shopping-cart fa-lg" ></i><span> {{ $shop->num_buy }} خرید از افزونه </span> </li>
                            <li><i class="fa fa-comments-o fa-lg" ></i><span> {{ $shop->num_comment }} دیدگاه </span> </li>
                            <li class="rate" ><div class="item-rate ltr" data-id="1" data-rating="3.5" ></div></li>
                        </ul>
                    </div>
                    <div class="content">
                        <p>{{ Config::get('addonShop.description') }}</p>

                        @foreach(Config::get('addonShop.attributes') as $index=>$attribute)
                            <div class="lists row">
                        ...
                        <div class="lists row">
                            ...
                            <div class="center-block">
                                <del class="discount">{{ number_format(Config::get('addonShop.base_price')) }} تومان  </del>
                            </div>
                        </div>
                        ...
                        <div class="lists row">
                            ...
                            <div class="center-block">
                                <div class="price">{{ number_format(Config::get('addonShop.base_price') - Config::get('addonShop.base_price')*Config::get('addonShop.discount')  ) }} تومان </div>
                                @if(Config::get('addonPoll.discount'))
                                    <div class="coupon"><span id="discount_amount">{{ number_format(Config::get('addonShop.base_price')*Config::get('addonShop.discount')) }}</span> تومان تخفیف | برای دریافت تخفیف ایمیل خود را تایید نمایید</div>
                                @endif
                            </div>
                        </div>
                    ...
                    <div class="comment-list">
                        @if(Auth::check())
                            <div class="new-comment">
                                ...
                                {!! Form::open(['route'=>['store.shop.comment'], 'method'=>'post']) !!}
                                <div class="media">
