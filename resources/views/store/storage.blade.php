        ...
        <div class="row">
            <div class="show-item">
                <div class="col-sm-4 pull-right">
                    <div class="image">
                        <img src="{{ asset('img/icons/store/'.Config::get('addonStorage.images')[0]) }}">
                    </div>
                    <div class="other-image">
                        <ul class="row">
                            @foreach(Config::get('addonStorage.images') as $image)
                ...
                <div class="col-sm-8 pull-right">
                    {!! Form::open(['route'=>'store.storage.buy', 'method'=>'get', 'id'=>'store_storage_form']) !!}
                    <div class="title">
                        <h3>{{ Config::get('addonStorage.title') }}</h3>
                        <ul class="info">
                            <li><i class="fa fa-shopping-cart fa-lg" ></i><span> {{ $storage->num_buy }} خرید از افزونه </span> </li>
                            <li><i class="fa fa-comments-o fa-lg" ></i><span> {{ $storage->num_comment }} دیدگاه </span> </li>
                            <li class="rate" ><div class="item-rate ltr" data-id="1" data-rating="3.5" ></div></li>
                        </ul>
                    </div>
                    <div class="content">
                        <p>{{ Config::get('addonStorage.description') }}</p>

                        @foreach(Config::get('addonStorage.attributes') as $index=>$attribute)
                            <div class="lists row">
                        ...
                        <div class="lists row">
                            ...
                            <div class="center-block">
                                <del class="discount" id="base_amount">{{ number_format(Config::get('addonStorage.base_price')) }} تومان  </del>
                            </div>
                        </div>
                        ...
                        <div class="lists row">
                            ...
                            <div class="center-block">
                                <div class="price" id="final_amount" >{{ number_format(Config::get('addonStorage.base_price') - Config::get('addonStorage.base_price')*Config::get('addonStorage.discount')  ) }} تومان </div>
                                @if(Config::get('addonStorage.discount'))
                                    <div class="coupon"><span id="discount_amount">{{ number_format(Config::get('addonStorage.base_price')*Config::get('addonStorage.discount')) }}</span> تومان تخفیف | برای دریافت تخفیف ایمیل خود را تایید نمایید</div>
                                @endif
                            </div>
                        </div>
                    ...
                    <div class="comment-list">
                        @if(Auth::check())
                            <div class="new-comment">
                                ...
                                {!! Form::open(['route'=>['store.storage.comment'], 'method'=>'post']) !!}
                                <div class="media">
